...
os.makedirs(f"./GPT_batches/{chart_type}", exist_ok=True)
df = pd.read_json(f"../dataset/{chart_type}/test_{generation_type}_{question_type}.json")
questions = df['query'].tolist()
gold_labels = df['label'].tolist()
imagenames = df['imgname'].tolist()
imagenames = [f"../dataset/{chart_type}/png/{imagename}" for imagename in imagenames]

payloads = [format_payload(f"{chart_type}_{question_type}_{generation_type}_{i}", prompt, questions[i], encode_image(imagenames[i])) for i in range(len(questions))]
for i, payload in enumerate(payloads):
    with open(f"./GPT_batches/{chart_type}/{question_type}_{generation_type}.jsonl", "a") as f:
        json.dump(payload, f)
        f.write("\n")
